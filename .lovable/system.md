...
- Права панель уроку — компонент `LessonSidebar`. Порядок завжди один:
  **таби → тіло (текст) → група статус-повідомлень (`callout`) → дії
  (`actions`)**. Не збирай власні картки для цього блоку.
- `callout` — це ГРУПА тіпсів (`Callout` / `Alert`) внизу панелі, одразу над
  кнопками: між ними мінімальний відступ `gap-2`, без роздільників. Передавай
  скільки завгодно станів у цей слот — вони автоматично злипаються в один блок
  і притискаються донизу, а головні кнопки (здача завдання, наступний урок,
  чат з ментором, матеріали) завжди залишаються НИЖЧЕ.
- Висота панелі й `ChatPanel` у стані `fit` однакова: `calc(100dvh - 2rem)`.
- `contentState="fit"` (дефолт) на десктопі робить панель липкою і не вищою за
  екран — скролиться лише тіло, сторінка не скролиться через панель.
  `"clamped"` обрізає тіло з градієнтом, `"full"` — показує весь контент,
  `"scroll"` — фіксована висота від батька. Інших станів немає.
- На мобільному дії панелі за замовчуванням стають floating-баром, закріпленим
...
- `ChatPanel` — панель чату з ментором (для студента й ментора однакова): та сама геометрія й логіка висоти, що в `LessonSidebar` (`fit` — sticky під висоту екрана, `scroll` — 100% висоти батька, `full` — росте з контентом), скролиться лише стрічка повідомлень, `composer` (`ChatInput`) закріплений знизу й на мобільному floating. Не робити власних чат-обгорток у проєктах.
...
- Визначення типу уроку та URL прев'ю — це бізнес-логіка LMS, а не дизайн-системи.
  Система дає пропси: передавай у `ContinueLessonCard` `kind` відповідно до типу
  контенту (`video` / `test` / `reading` / `task` / `image`) і `previewVideoSrc`
  для відео-уроків (пряме відео чи YouTube-посилання). `MediaPreview` сам вирішує
  порядок: реальний кадр відео → YouTube-мініатюра → передане зображення `src` →
  гліф-заглушка за `kind`. Якщо урок не відео — `previewVideoSrc` не передавай,
  залиш лише `kind`.
- У превʼю рейтингу передавай топ-список через `entries`, а поточного студента
  окремо через `currentEntry`. `LeaderboardCard` сам підсвітить його в списку
  або додасть гарантовано видимим окремим рядком унизу без дублювання.
- Бали, час і місця передаються вже відформатованими рядками — компоненти не
  форматують числа.

## Картки курсів

- `CourseCard` — самодостатня біла поверхня на сірому полотні; не обгортай її локальним `Card` і не прибирай `bg-surface`.
- Порядок даних фіксований: кавер із тегами → назва й опис → прогрес → `meta` (модулі / уроки / тести) → `action` → `mentors`.
- У `mentors` передавай тільки `UserChip`: підпис «Ментори навчання» та чипи завжди лишаються однією групою внизу картки. Не змішуй туди метадані або статус доступу.
- «Доступ завершено» передавай тільки через `coverStatus`. Компонент затемнює кавер і показує статус першою пігулкою у верхньому рядку тегів — під кавером він не рендериться; не дублюй його в `coverTags`, `meta` чи `details`.
- Теги доступу й супроводу («Доступ до 12 бер. 27'», «Пожиттєвий доступ», «Ментор до 12 бер. 26'», «Пожиттєвий супровід», «Супровід закінчився», «Самостійне навчання») передавай тільки через `coverTags` як `Badge tone="onMuted"` — вони стоять зверху на кавері. Не рендери ці дані під кавером або через `details`. Який саме тег показувати — бізнес-логіка LMS (дати, тариф з ментором чи без); картка лише позиціонує теги.
- Кавер завжди займає ширину картки й зберігає пропорцію 26:15; не додавай локальні `h-*`, `w-*` або `aspect-*`.
...
- Подкасти — це YouTube-відео. Жодних завантажень файлів, жодного власного
  плеєра й жодного прямого `<iframe>` у LMS.
- Картка матеріалу-подкасту — `PodcastCard`: обкладинка через `coverVideoSrc`
  (YouTube-мініатюра), `episodeCount`, `progress` («Прослухано 40%»),
  згортний список `EpisodeRow` та слот `player`. Картка завжди має власну білу
  поверхню `bg-surface`, скруглення й внутрішні відступи — не додавай локальну
  обгортку та не покладайся на фон сторінки.
- Обкладинка та інлайн-плеєр займають одну й ту саму сцену 16:9 (`MediaPreview
  size="video"`), тому запуск епізоду не змінює висоту картки. Не задавай
  плеєру власну висоту в LMS.
- Іконка подкасту (навушники на акцентному фоні) за замовчуванням у правому
  верхньому кутку обкладинки. Не додавай власну іконку поверх.
- Ширина картки — від контейнера (`w-full`). За список матеріалів відповідає
  сітка LMS: з одним подкастом обмеж колонку (напр. `max-w-sm`), щоб картка не
  розтягувалась на весь екран.
- Один епізод (`episodeCount === 1`) — компактний режим: `EpisodeRow` і контрол
  «Показати епізоди» не рендеряться взагалі; play/pause — круглий `IconButton`
  (`play`/`pause`) ліворуч від `ProgressBar` у блоці прогресу. Керується пропами
  `playing` / `onTogglePlay` / `startSecond` (діють лише в цьому режимі); без
  `onTogglePlay` кнопка не показується. `playing` синхронізуй з `YouTubePlayer`
  у слоті `player` — кнопка керує тим самим відтворенням. Мульти-епізодні
  подкасти не змінюються: play лишається в `EpisodeRow`, прогрес без кнопки.
- Епізод — `EpisodeRow` (номер / галочка «прослухано», назва, тривалість,
  play/pause). Одночасно `playing` може бути лише в одного епізоду на сторінці.
- Плеєр — `YouTubePlayer` (YouTube IFrame API). Керується пропом `playing`,
  стартує з `startSecond`, а `onTimeChange` / `onDuration` / `onEnded` — це
  джерело даних для прогресу. Недоступне/видалене відео показує вбудований
  стан помилки. Стандартні контролі, клавіатурне керування, fullscreen,
  субтитри та анотації YouTube вимкнені; iframe пасивний, а play/pause живе в
  `EpisodeRow`. YouTube може залишити обов'язковий брендинг — не маскуй його.
- `audioOnly` у `YouTubePlayer` — аудіо-режим (кадр приховано, лишається
  компактна смужка з підписом).
- `MediaPreview` має `kind="audio"` (глиф навушників) для подкаст-плашок.
- LMS відповідає за дані: таблиці, RLS, збереження `current_second`
  (debounce ~5с), `completed` на `onEnded` або ~90% тривалості, пауза
  попереднього епізоду при старті нового. Дизайн-система нічого не фетчить.
