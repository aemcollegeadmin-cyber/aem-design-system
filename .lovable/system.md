...
- Кожна сторінка починається з `PageNav`: кнопка «назад», заголовок (H1) і
  другорядний рядок — `subtitle` або `breadcrumbs`. Не збирай власні шапки з
  IconButton + тексту, щоб верх не «стрибав» між сторінками.
- `backHref` = навігація (рендериться `<a>`), `onBack` = дія (`<button>`).
  Опускай обидва тільки на кореневих сторінках без батька.
- Статус або дії сторінки йдуть у проп `actions` (справа).
