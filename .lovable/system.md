...
- Порядок даних фіксований: кавер → назва й опис → прогрес → `meta` (модулі / уроки / тести) → `action` → `details` (доступ / формат) → `mentors`.
...
- «Доступ завершено» передавай тільки через `coverStatus`. Компонент сам затемнює кавер і накладає статус поверх нього; не дублюй цей статус серед `meta` чи `details`.
- Теги доступу й супроводу («Доступ до 12 бер. 27'», «Пожиттєвий доступ», «Ментор до 12 бер. 26'», «Супровід закінчився», «Самостійне навчання») передавай через `coverTags` як `Badge tone="onMuted"` — вони стоять зверху на кавері. Не дублюй цю інформацію в `details`: там лишається лише формат навчання (онлайн, власний темп). Який саме тег показувати — бізнес-логіка LMS (дати, тариф з ментором чи без); картка лише позиціонує теги.
