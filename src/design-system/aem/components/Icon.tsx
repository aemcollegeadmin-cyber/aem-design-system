import { forwardRef } from "react";
import {
  Activity,
  ArrowDown,
  ArrowLeft,
  ArrowRight,
  ArrowUp,
  Award,
  Ban,
  Bell,
  Bold,
  BookOpen,
  Bookmark,
  Camera,
  Calendar,
  Check,
  ChevronDown,
  ChevronLeft,
  ChevronRight,
  ChevronUp,
  Circle,
  CircleUser,
  ClipboardCheck,
  CircleAlert,
  CircleCheck,
  Clock,
  Copy,
  Download,
  Ellipsis,
  Eye,
  EyeOff,
  ExternalLink,
  File,
  FileImage,
  FileText,
  Folder,
  FolderOpen,
  GraduationCap,
  GripVertical,
  Heading1,
  Heading2,
  Image,
  ImagePlus,
  Info,
  Italic,
  KeyRound,
  LayoutGrid,
  Link,
  List,
  ListOrdered,
  Loader,
  Flame,
  LogIn,
  LogOut,
  Mail,
  Megaphone,
  Menu,
  MessageCircle,
  Minus,
  PartyPopper,
  Pencil,
  Play,
  Plug,
  Plus,
  Quote,
  RefreshCw,
  RotateCcw,
  Save,
  Search,
  Settings,
  SlidersHorizontal,
  Sparkles,
  SquareCheck,
  Star,
  Trash,
  TriangleAlert,
  Trophy,
  Unlink,
  Unplug,
  Upload,
  User,
  UserCog,
  UserMinus,
  UserPlus,
  Users,
  Wand,
  X,
  ZoomIn,
  ZoomOut,
  type LucideIcon,
} from "lucide-react";
import * as Ph from "@phosphor-icons/react";
import { cn } from "../lib/cn";


/**
 * Curated icon registry. Every entry is a geometrically simple outline glyph
 * (single shape, no small internal details) so icons stay legible at 16px and
 * never add visual noise. Add new semantic names here instead of importing
 * lucide icons directly in product code.
 */
export const icons = {
  /* Navigation */
  home: LayoutGrid,
  menu: Menu,
  more: Ellipsis,
  back: ArrowLeft,
  forward: ArrowRight,
  up: ArrowUp,
  down: ArrowDown,
  chevronLeft: ChevronLeft,
  chevronRight: ChevronRight,
  chevronUp: ChevronUp,
  chevronDown: ChevronDown,
  external: ExternalLink,
  close: X,
  logout: LogOut,

  /* Learning */
  course: Folder,
  module: Folder,
  lesson: File,
  document: File,
  review: SquareCheck,
  bookmark: Bookmark,
  star: Star,

  /* People & communication */
  user: User,
  users: Users,
  chat: MessageCircle,
  mail: Mail,
  bell: Bell,

  /* Actions */
  add: Plus,
  remove: Minus,
  edit: Pencil,
  delete: Trash,
  copy: Copy,
  search: Search,
  settings: Settings,
  send: ArrowUp,
  play: Play,
  show: Eye,
  hide: EyeOff,

  /* Status */
  check: Check,
  done: CircleCheck,
  alert: CircleAlert,
  info: Info,
  dot: Circle,
  clock: Clock,
  calendar: Calendar,
  warning: TriangleAlert,
  locked: Ban,
  loader: Loader,
  flame: Flame,

  /* Content & editor */
  bold: Bold,
  italic: Italic,
  heading1: Heading1,
  heading2: Heading2,
  list: List,
  listOrdered: ListOrdered,
  quote: Quote,
  link: Link,
  unlink: Unlink,
  gripVertical: GripVertical,
  save: Save,

  /* Files & media */
  bookOpen: BookOpen,
  fileText: FileText,
  fileImage: FileImage,
  folderOpen: FolderOpen,
  image: Image,
  imagePlus: ImagePlus,
  camera: Camera,
  upload: Upload,
  download: Download,
  zoomIn: ZoomIn,
  zoomOut: ZoomOut,

  /* Learning & achievements */
  graduationCap: GraduationCap,
  award: Award,
  trophy: Trophy,
  clipboardCheck: ClipboardCheck,
  activity: Activity,
  megaphone: Megaphone,
  partyPopper: PartyPopper,
  sparkles: Sparkles,
  wand: Wand,

  /* Accounts & system */
  userCircle: CircleUser,
  userCog: UserCog,
  userPlus: UserPlus,
  userMinus: UserMinus,
  logIn: LogIn,
  keyRound: KeyRound,
  sliders: SlidersHorizontal,
  refresh: RefreshCw,
  rotateCcw: RotateCcw,
  plug: Plug,
  unplug: Unplug,
} satisfies Record<string, LucideIcon>;

/**
 * Two-tone counterparts used for the small sizes (sm/md), where a 2px outline
 * stroke is visually too heavy. Rendered with Phosphor's `duotone` weight: a
 * solid primary shape in `currentColor` plus a paired translucent secondary
 * shape. Same semantic names as `icons`.
 */
export const filledIcons = {
  home: Ph.SquaresFour,
  menu: Ph.List,
  more: Ph.DotsThree,
  back: Ph.ArrowLeft,
  forward: Ph.ArrowRight,
  up: Ph.ArrowUp,
  down: Ph.ArrowDown,
  chevronLeft: Ph.CaretLeft,
  chevronRight: Ph.CaretRight,
  chevronUp: Ph.CaretUp,
  chevronDown: Ph.CaretDown,
  external: Ph.ArrowSquareOut,
  close: Ph.X,
  logout: Ph.SignOut,

  course: Ph.Folder,
  module: Ph.Folder,
  lesson: Ph.File,
  document: Ph.File,
  review: Ph.CheckSquare,
  bookmark: Ph.BookmarkSimple,
  star: Ph.Star,

  user: Ph.User,
  users: Ph.Users,
  chat: Ph.ChatCircle,
  mail: Ph.Envelope,
  bell: Ph.Bell,

  add: Ph.Plus,
  remove: Ph.Minus,
  edit: Ph.PencilSimple,
  delete: Ph.Trash,
  copy: Ph.Copy,
  search: Ph.MagnifyingGlass,
  settings: Ph.Gear,
  send: Ph.ArrowUp,
  play: Ph.Play,
  show: Ph.Eye,
  hide: Ph.EyeSlash,

  check: Ph.Check,
  done: Ph.CheckCircle,
  alert: Ph.WarningCircle,
  info: Ph.Info,
  dot: Ph.Circle,
  clock: Ph.Clock,
  calendar: Ph.CalendarBlank,
  warning: Ph.Warning,
  locked: Ph.Prohibit,
  loader: Ph.CircleNotch,

  bold: Ph.TextB,
  italic: Ph.TextItalic,
  heading1: Ph.TextHOne,
  heading2: Ph.TextHTwo,
  list: Ph.List,
  listOrdered: Ph.ListNumbers,
  quote: Ph.Quotes,
  link: Ph.Link,
  unlink: Ph.LinkBreak,
  gripVertical: Ph.DotsSixVertical,
  save: Ph.FloppyDisk,

  bookOpen: Ph.BookOpen,
  fileText: Ph.FileText,
  fileImage: Ph.FileImage,
  folderOpen: Ph.FolderOpen,
  image: Ph.Image,
  imagePlus: Ph.ImageSquare,
  camera: Ph.Camera,
  upload: Ph.UploadSimple,
  download: Ph.DownloadSimple,
  zoomIn: Ph.MagnifyingGlassPlus,
  zoomOut: Ph.MagnifyingGlassMinus,

  graduationCap: Ph.GraduationCap,
  award: Ph.Medal,
  trophy: Ph.Trophy,
  clipboardCheck: Ph.ClipboardText,
  activity: Ph.Pulse,
  megaphone: Ph.Megaphone,
  partyPopper: Ph.Confetti,
  sparkles: Ph.Sparkle,
  wand: Ph.MagicWand,

  userCircle: Ph.UserCircle,
  userCog: Ph.UserGear,
  userPlus: Ph.UserPlus,
  userMinus: Ph.UserMinus,
  logIn: Ph.SignIn,
  keyRound: Ph.Key,
  sliders: Ph.Sliders,
  refresh: Ph.ArrowsClockwise,
  rotateCcw: Ph.ArrowCounterClockwise,
  plug: Ph.Plug,
  unplug: Ph.Plugs,
  flame: Ph.Fire,
} satisfies Record<keyof typeof icons, Ph.Icon>;


export type IconName = keyof typeof icons;

/**
 * Icon scale.
 * - `md` (20) renders two-tone (duotone) glyphs — a 2px stroke is far too
 *   heavy at that size, so small icons are solid shapes instead.
 * - `lg` (24) and `xl` (32) render outline glyphs with a constant 2px stroke.
 */
export const iconSizes = { md: 20, lg: 24, xl: 32 } as const;

export type IconSize = keyof typeof iconSizes;

/** Sizes rendered as two-tone glyphs. */
const filledSizes: IconSize[] = ["md"];

/**
 * Glyphs that have no enclosed area (arrows, carets, cross, plus …). Phosphor's
 * `fill` weight wraps those in a solid square, which reads as a different icon,
 * so they use the heavy `bold` weight instead — visually solid, same shape.
 */
const strokeOnlyGlyphs = new Set<IconName>([
  "menu",
  "more",
  "back",
  "forward",
  "up",
  "down",
  "chevronLeft",
  "chevronRight",
  "chevronUp",
  "chevronDown",
  "external",
  "close",
  "logout",
  "add",
  "remove",
  "edit",
  "copy",
  "search",
  "send",
  "check",
  "list",
  "listOrdered",
  "gripVertical",
  "italic",
  "link",
  "unlink",
  "loader",
  "refresh",
  "rotateCcw",
  "upload",
  "download",
  "sliders",
]);


export interface IconProps extends Omit<React.SVGProps<SVGSVGElement>, "ref"> {
  /** Semantic icon name from the system registry. */
  name: IconName;
  /** md 20px is two-tone; lg 24px & xl 32px are 2px outline. */
  size?: IconSize;
  /** Accessible name. Omit for purely decorative icons. */
  label?: string;
}

/**
 * The single way to render an icon in this system: two-tone glyphs at md,
 * simple outline glyphs with a constant 2px stroke at lg/xl.
 */
export const Icon = forwardRef<SVGSVGElement, IconProps>(function Icon(
  { name, size = "lg", label, className, ...props },
  ref,
) {
  const px = iconSizes[size];
  const filled = filledSizes.includes(size);
  const a11y = {
    "data-aem-icon": name,
    "aria-hidden": label ? undefined : true,
    "aria-label": label,
    role: label ? ("img" as const) : undefined,
    className: cn("shrink-0", className),
  };

  if (filled) {
    const Solid = filledIcons[name];
    return (
      <Solid
        ref={ref}
        size={px}
        weight={strokeOnlyGlyphs.has(name) ? "bold" : "duotone"}
        {...a11y}
        {...(props as Record<string, unknown>)}
      />
    );
  }

  const Glyph = icons[name];
  return (
    <Glyph
      ref={ref}
      width={px}
      height={px}
      strokeWidth={2}
      absoluteStrokeWidth
      {...a11y}
      {...props}
    />
  );
});

